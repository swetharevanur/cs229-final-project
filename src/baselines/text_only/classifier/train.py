...
sys.stdout = open(os.path.join('output', 'logs', 'lstm_tuned.log'), 'a+')
...
def train_model(model, train_batcher, val_batcher, optimizer, scheduler, criterion, dataset_sizes, hyperparams):
	model.train()
...
def train_handler(hyperparams, text_datasets, TEXT, LABEL, num_tokens):
	...
	embedding = nn.Embedding(num_tokens, hyperparams['embedding_size'], padding_idx = 1, max_norm = 1)
	...
	optimizer = torch.optim.SGD(model.parameters(), hyperparams['init_lr'], momentum = 0.9)

	# decay LR by a factor of 0.1 every 7 epochs
	exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size = 7, gamma = 0.1)

	return train_model(model, train_batcher, val_batcher, optimizer, exp_lr_scheduler,
